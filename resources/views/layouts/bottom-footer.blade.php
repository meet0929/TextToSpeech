 <div class="row light-bg p-3 my-4 d-flex align-items-center m-1">
                ...
                <div class="col-md-8 col-12 d-lg-flex align-items-center mb-3 mb-md-0">
    ...
    <script>
        window.location.href = 'speech-synthesis';
    </script>
    @endauth
    @endif
    @if(Route::currentRouteName() == 'home')
    <script>
        ...
        function playPauseaudio(audioId) {
            ...
            var audioPlayer = document.getElementById(audioId).children[0];
                
                if (audioPlayer.paused) {
                    var playPauseBtn = document.getElementById(audioId).querySelector('.play-btn-2');
                    // If audio is paused, play it
                    audioPlayer.play();
                    playPauseBtn.classList.remove('play-btn-2');
                    playPauseBtn.classList.add('pause-btn-2');
                } else {
                    var playPauseBtn = document.getElementById(audioId).querySelector('.pause-btn-2');
                    // If audio is playing, pause it
                    audioPlayer.pause();
                    playPauseBtn.classList.remove('pause-btn-2');
                    playPauseBtn.classList.add('play-btn-2');
                }

                // Pause other audio elements
                var audioBlocks = document.querySelectorAll('.audio-block');
                
                audioBlocks.forEach(function(block) {
                    
                    var otherAudioId = block.id;
                    var otherAudioPlayer = block.querySelector('.audio-player');
                    var otherPlayPauseBtn = block.querySelector('.pause-btn-2');
                    if (otherAudioId !== audioId && otherPlayPauseBtn) {
                        otherAudioPlayer.pause();
                        otherPlayPauseBtn.classList.remove('pause-btn-2');
                        otherPlayPauseBtn.classList.add('play-btn-2');
                    }
                });
        }
        ...
        function playPause(audioElement) {
            ...
            var audioContainer = aud.closest('.audio-container'); // Find the container of the audio element and the button
            // var aud = audioContainer.querySelector('.aud-element'); // Find the audio element within the container
            // console.log(event.target);
            var className = aud.className;
            let playBtnclass = className.replace('aud-element play', "");
            var isPlaying = !aud.paused;
            var playPauseBtn = audioContainer.querySelector('.pause-btn-'+playBtnclass);
            
            if (isPlaying) {
                aud.pause();
                playPauseBtn.classList.remove('pause-btn-'+playBtnclass);
                playPauseBtn.classList.add('play-btn-'+playBtnclass);
            } else {
                var playPauseBtn = audioContainer.querySelector('.play-btn-'+playBtnclass);
                if (currentAudio && currentAudio !== aud) {
                    currentAudio.pause();
                    currentAudio.currentTime = 0;
                    let curAudClass = currentAudio.className;
                    let curPlayAudclass = curAudClass.replace('aud-element play', "");
                    const currentButton = currentAudio.nextElementSibling;
                    currentButton.classList.remove('pause-btn-'+curPlayAudclass);
                }
                aud.play();
                playPauseBtn.classList.add('pause-btn-'+playBtnclass);
                currentAudio = aud;
            }
        ...
        const appendAlert = (message, type) => {
            ...
            wrapper.innerHTML = [
                `<div class="alert alert-${type} forgot-password-alert d-block mx-auto" role="alert">`,
                `   <div>${message}</div>`,
        ...
        if (alertTrigger) {
            alertTrigger.addEventListener('click', () => {
                appendAlert('If a user with the email <b>aaaaaaaaa@bbb.com</b> exists, then we have sent an email containing instructions on how to reset your password.', 'dark')
            })
        }

        document.addEventListener('DOMContentLoaded', (event) => {
            const scrollSection = document.querySelector('.benefit-scroll-section');
            const sections = document.querySelectorAll('.benefit-div');
            let currentSectionIndex = 0;
            function updateBackgroundImage(index) {
                const newBackgroundImage = sections[index].getAttribute('data-bg');
                scrollSection.style.backgroundImage = `url(${newBackgroundImage})`;
            }
            scrollSection.addEventListener('scroll', () => {
                const scrollTop = scrollSection.scrollTop;
                const sectionHeight = sections[0].clientHeight + parseInt(window.getComputedStyle(sections[0]).marginBottom);
                const newSectionIndex = Math.floor(scrollTop / sectionHeight);
                if (newSectionIndex !== currentSectionIndex) {
                    sections[currentSectionIndex].classList.add('hidden');
                    sections[newSectionIndex].classList.remove('hidden');
                    currentSectionIndex = newSectionIndex;
                    updateBackgroundImage(currentSectionIndex);
                }
            });
